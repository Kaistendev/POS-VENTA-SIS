...
export function setupIpcHandlers() {
   ...
   * DISCOUNTS
   */
  ipcMain.handle("discounts:getAll", async (_, activeOnly?: boolean) => {
    try {
      const user = getCurrentUser();
      if (!user) throw new UnauthorizedError();
  ...
  ipcMain.handle("discounts:getById", async (_, id: number) => {
    try {
      const user = getCurrentUser();
      if (!user) throw new UnauthorizedError();
  ...
  ipcMain.handle("discounts:update", wrapIpc(requireRole('ADMIN')(async (id: number, data) => {
    const parsed = discountSchema.partial().parse(data);
    return await $.discountService.updateDiscount(id, parsed);
  })));

  ipcMain.handle("discounts:delete", wrapIpc(requireRole('ADMIN')(async (id: number) => {
    return await $.discountService.deleteDiscount(id);
  })));

  ipcMain.handle("discounts:getApplicable", async (_, productId: number, totalAmount?: number) => {
    try {
      const user = getCurrentUser();
      if (!user) throw new UnauthorizedError();

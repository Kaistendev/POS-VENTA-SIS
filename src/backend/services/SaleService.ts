...
import { saleSchema } from '../../shared/schemas.js';
...
export class SaleService {
  ...
  async registerSale(saleData: any, itemsData: any[], userId: number = 1) {
    ...
    const itemsWithPurchasePrice = await Promise.all(validated.items.map(async (item: any) => {
      const product = productMap.get(item.product_id);
    ...
    const totalDiscount = itemsWithPurchasePrice.reduce(
      (acc: number, item: any) => acc + (item.discount_amount || 0),
      0,
    );

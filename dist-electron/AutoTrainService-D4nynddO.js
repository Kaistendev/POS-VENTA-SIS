...
var IntentClassifierTrainer = class {
	...
	async train(config = {}, epochs = 200, batchSize = 8, onEpochEnd) {
		...
		logger.info({
			...
			valSize: xVal?.shape[0] || 0
		}, "Starting model.fit (train method)");
		...
		logger.info({
			...
			xTrainSample: xTrainData[0]?.slice(0, 5),
			yTrainSample: yTrainData[0]
		}, "Input data check (train method)");
		...
		logger.info({
			labelCounts,
			intentNames: Object.values(INTENT_TO_INDEX).sort((a, b) => INTENT_TO_INDEX[a] - INTENT_TO_INDEX[b])
		}, "Label distribution");
		let history;
		try {
			...
			logger.info({
				...
				historyLen: history.history.loss?.length
			}, "model.fit completed (train method)");
		} catch (err) {
